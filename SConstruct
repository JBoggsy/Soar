...
print("   Dependencies:      --use-ros --use-opencv")
...
if compiler == 'g++':
    ...
    if GetOption('useopencv'):
        ...
        libs += ['opencv_core', 'opencv_imgproc', 'opencv_highgui', 'opencv_imgcodecs', 'opencv_ximgproc']
    if GetOption('useros'):
        cflags.append('-DENABLE_ROS')
        libs += ['roscpp', 'libpcl_common']
    if GetOption('usetorch'):
        cflags.append('-DENABLE_TORCH')
        libs += ['torch', 'torch_cpu', 'torch_cuda', 'torch_global_deps']
