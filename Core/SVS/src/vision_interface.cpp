...
void vision_interface::proxy_get_children(std::map<std::string, cliproxy*>& c) {
    ...
    c["inject"] =  new memfunc_proxy<vision_interface>(this, &vision_interface::cli_inject);
...
void vision_interface::cli_inject(const std::vector<std::string>& args, std::ostream& os) {
    if (args.empty()) {
    ...
    std::string dec_jpg = base64_decode(img_data);
    std::vector<uchar> data(dec_jpg.begin(), dec_jpg.end());
    cv::Mat img = cv::imdecode(cv::Mat(data), 1);

    _svs_ptr->image_callback(img);
...
void vision_interface::cli_export_imagination(const std::vector<std::string>& args, std::ostream& os) {
    ...
    opencv_image* imagined_percept = _svs_ptr->get_root_state()->get_vwm()->get_percept();
